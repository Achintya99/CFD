...
int main()
{
	...
	for (int t = 0;t < ts;t++)
	{
		...
		do
		{
			...
			for (int i = 1;i < ny - 1;i++)
			{
				...
				{
					b[i][j] = Tn[i + 1][j] + Tn[i - 1][j] + Tn[i][j + 1] + Tn[i][j - 1]+To[i][j+1]+To[i][j-1]+To[i-1][j]+To[i+1][j];//updating b after each iteration
					Tn[i][j] = (((1 - 2 * lambda) * To[i][j]) + (lambda * b[i][j] / 2)) / (1 + (2 * lambda));
	...
	fstream ofile;
	ofile.open("C:/Users/ACHINTYA/Desktop/Mini_Project/Trial_crank_nicolson.csv", ios::out);
	//end of time loop
	ct = clock() - ct;
	...
	cout << "\nIterations= " << g << " time= " << double(ct / CLOCKS_PER_SEC);
	//putting in a dat file as well
	fstream op;
	op.open("C:/Users/ACHINTYA/Desktop/Mini_Project/TrialCN.dat", ios::out);
	for (int j = 0;j < nx;j++)
	{
		for (int i = 0;i < ny;i++)
		{
